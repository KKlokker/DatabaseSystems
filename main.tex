...
<!DOCTYPE Stars [
	...
	\section{Algebraic Query Language}
		...
		\subsection{Examples}
			...
			\end{minipage}
	\section{Transaction}
		A transaction is a collection of queries which is executed atomicly.\\
		This is to prevent race conditions in case of read then write.\\
		The transaction is also used such it can both end up commiting or rollback changes it has done.\\
		...
		\beginSQL 
			...
			ROLLBACK
		\end{lstlisting}
		A transaction can also have different proberties.
		This includes it being able to read only or red write. Where read only makes it possible for more concurrency.\\
		This is set by \beginSQL
		SET TRANSACTION READ ONLY \end{lstlisting} OR READ WRITE in the other case, but this is default. \\
		This is added like other attributes before the transaction starts.\\[4mm]
		For a transaction there are 3 different terms which is used for isolations levels:\\
		\begin{itemize}
				  \item Dirty reads
		...
		Phantom tubles are tubles which are inserted while a transaction is ongoing.\\[4mm]
		There are 4 diffferent isolations levels:
		\begin{itemize}
		...
		Read uncommited allows dirty reads and phantoms and is nonrepeatable.\\
		To set a transaction level it can be done like:
		\beingSQL
		 SET TRANSACTION ISOLATION LEVEL REPEATABLE READ;
		\end{lstlisting}
		DISCLAIMER: if dirty reads are allowed the default is read only and not read write.
	...
	\section{Stored Procedures}
		...
		They are defined by:\\
		\beginSQL
		CREATE PROCEDURE <name> (IN <name> <type>..., OUT <type>)
			.
			.
			.
		\end{lstlisting}
		In case of a return is is:
		\beginSQL
		CREATE PROCEDURE <name> (<params>) RETURN <type>
		\end{lstlisting}
		The body of the procedure can be SQL commands and other forms of command listed below.
		\subsection{Variables}
			Variables are done by:
			\beginSQL
			DECLARE <name> <type>;
			SET <name> = <expression>;
	 		\end{lstlisting}
		...
		\subsection{If statements}
			If statements are build up in the following form:
			\beginSQL
			IF <condition> THEN
			...
			...
			END IF;
 			\end{lstlisting}
			Where conditions are done through sql queries like NOT EXISTS and COUNT.
		\subsection{Cursor}
			A cursor is a query which points towardsa a DB and can be used to fetch tubles.\\
			\beginSQL
			DECLARE <name> CURSOR FOR SELECT <attributes> FROM <DB> WHERE ... 
			\endSQL
		\subsection{Loop}
			The loop in itself is really simple consisting of:
			\beginSQL 
			<label>: LOOP
				...
			END LOOP;
	 		\end{lstlisting}
			As seen there is no guard so therefore an if condition is done itself to leave the loop.\\
			The most common use case is looping through a DB, to create a leave if a variable is used for when a cursor hits the end:
			\beginSQL
			DECLARE Not_Found CONDITION FOR SQLSTATE '02000';
			\end{lstlisting}
			Where 02000 is a SQL state representing null in a select.\\
			So a loop which exit when a cursor hits the end is:
			\beginSQL
			DECLARE age INTEGER;
			<label>: LOOP
				FETCH FROM <cursorVariable> INTO age;
				IF Not_Found THEN LEAVE <label> END IF;
				...
			END LOOP;
 			\end{lstlisting}
		\subsection{For loops}
			...
			END FOR;
 			\end{SQL}
			Here in the query the attributes listed after the select are the variable names used in the do.\\
		\subsection{Exception handlers}
			...
			\end{lstlisting}
			The where to go can be 3 options which determine the outcome after executing the statement:\\
			...
			An example for a handler can be:
			\beginSQL
			DECLARE Too$\_$Many CONDITION FOR SQLTATE '21000';
			BEGIN
				DELCARE EXIT HANDLER FOR Too$\_$Many RETURN NULL;
				...
			END;
	\section{Call-level Interface}
